...
def set_all_location_rules(world: LethalCrisisWorld):
    ...
    world.set_rule(
        world.get_location(rank_location_name(6, "A")),
        partial(has_max_life, life_target=1200, player=world.player)
    )
    world.set_rule(
        world.get_location(rank_location_name(9, "A")),
        partial(has_max_life, life_target=1600, player=world.player)
    )
    world.set_rule(
        world.get_location(rank_location_name("13B", "S")),
        partial(has_max_life, life_target=3000, player=world.player)
    )

    # 7-S requires a specific set of applications equipped
    world.set_rule(
        world.get_location(rank_location_name(7, "S")),
        CAN_ENTER_SECRET_ROOM
    )

    # All ranks in 7B require the secret room, same as 7-S
    world.set_rule(
        world.get_location(rank_location_name("7B", "S")),
        CAN_ENTER_SECRET_ROOM
    )
    world.set_rule(
        world.get_location(rank_location_name("7B", "A")),
        CAN_ENTER_SECRET_ROOM
    )
    world.set_rule(
        world.get_location(rank_location_name("7B", "B")),
        CAN_ENTER_SECRET_ROOM
    )
    world.set_rule(
        world.get_location(rank_location_name("7B", "C")),
        CAN_ENTER_SECRET_ROOM
    )
    world.set_rule(
        world.get_location(rank_location_name("7B", "D")),
        CAN_ENTER_SECRET_ROOM
    )

    # 9-S requires one specific application
    world.set_rule(
        world.get_location(rank_location_name(9, "S")),
        Has(names.virus_transform)
    )
